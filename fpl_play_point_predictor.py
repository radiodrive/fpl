...
def feature_engineering(df):
    ...
    # --- THIS IS THE CORRECTED FEATURE LIST ---
    # It no longer includes 'position_DEF' and now includes 'position_GK'
    feature_cols = [f'{s}_last_{past_window}' for s in stats_to_roll] + \
                   ['value', 'position_FWD', 'position_GK', 'position_MID']
    
    # Ensure all columns exist before selecting them, in case a category is missing in the data
    for col in feature_cols:
        if col not in df.columns:
            df[col] = 0 # If a column is missing (e.g., no GKs in data), create it with all zeros
